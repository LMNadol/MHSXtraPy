...
class PlotCut:
    ...
    def __init__(
        ...
        self.ax.set_xticks(
            np.linspace(0, 2 * np.pi, nticks + 1), latex_ticks(nticks, 2)
        )
        ...
        plotfield = self.br[ir, :, :] + (self.plane_d - self.rads[ir]) / (
            self.rads[ir + 1] - self.rads[ir]
        ) * (self.br[ir + 1, :, :] - self.br[ir, :, :])
        contours = self.ax.contourf(
            self.phis,
            self.thetas,
            plotfield,
            levels,
    ...
    def spines(self, labels=False, ms=3):
        ...
        for inull, spine in zip(null_nums, spines):
            self.ax.plot(
                spine[2], spine[1], ".", c=self.colours[inull - 1], ms=ms, zorder=-5
            )
            if labels:
    ...
    def separators(self, labels=False, ms=3, hcs=False):
        ...
        """
        null_nums, separators = rd.cut_separators(
            self.datafile, self.plane_norm, self.plane_d, hcs=hcs
        )
    ...
    def rings(self, dots=False, labels=False, lw=1):
        ...
        for inull, ring in zip(null_nums, rings):
            self.ax.plot(
                ring[:, 2], ring[:, 1], c=self.colours[inull - 1], lw=lw, zorder=-5
            )
            if labels:
                self.ax.text(
                    ring[ring.length[0] // 2, 2],
                    ring[ring.length[0] // 2, 1],
                    "{}".format(inull),
                )
            if dots:
    ...
    def nulls(self, labels=False, size=1, zorder=-1):
        ...
        for i in range(self.nulldata.shape[0]):
            self.ax.plot(
                self.nulldata.pos[i, 2],
                self.nulldata.pos[i, 1],
                ".",
            ...
            if labels == True:
                self.ax.text(
                    self.nulldata.pos[i, 2],
                    self.nulldata.pos[i, 1],
                    "{}, {:04f}".format(i, self.nulldata.pos[i, 0]),
                )
    ...
    def get_colours(self):
        ...
        negcolors = [
            negcolors(i)
            for i in np.linspace(0.5, 1, np.count_nonzero(self.nulldata.sign == -1))
        ]
        poscolors = [
            poscolors(i)
            for i in np.linspace(0.5, 1, np.count_nonzero(self.nulldata.sign == 1))
