...
def plot(
    ...
    nullpos = (
        nulls[n - 1].gridpos - 1
    )  # -1 translation from fortran to idl grid spacing
    ...
    if converge:
        ...
        fanpts = np.array(
            [
                (max1 * np.cos(angle) + min1 * np.sin(angle)) * rsphere + nullpos
                for angle in angles
            ]
        )
    ...
    for i in range(3):
        boxedge[:, i] = np.array(
            [nullpos[i] - draw_scale * rsphere, nullpos[i] + draw_scale * rsphere]
        )
    boxedge = np.array(
        [
            [nullpos[i] - draw_scale * rsphere, nullpos[i] + draw_scale * rsphere]
    ...
    if fan == True:
        ...
        startpts_fan = [
            rad * (max1 * np.cos(phi[i]) + min1 * np.sin(phi[i])) for phi in phis
        ]
    ...
    if ball:
        ...
        startpts_ball = [
            np.array(
                [
                    rad * np.cos(ip) * np.sin(it),
                    rad * np.sin(ip) * np.sin(it),
                    rad * np.cos(it),
                ]
            )
            for it in thetas
